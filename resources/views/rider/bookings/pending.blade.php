...
@section('content')
    <div class="btn-group ms-5 mt-5" role="group" aria-label="Basic example">
        <a href="{{ route('rider.bookings.pending') }}">
            <button type="button" class="btn btn-primary">Ready for Pickup</button>
        </a> 
        <a href="{{ route('rider.bookings.pick-up') }}">
            <button type="button" class="btn mx-1 btn-secondary">Pick-up</button>
        </a> 
        <a href="{{ route('rider.bookings.process') }}">
            <button type="button" class="btn mx-1 btn-secondary">DElivery</button>
        </a> 
        {{-- <a href="{{ route('rider.bookings.delivery') }}">
            <button type="button" class="btn btn-secondary">Delivery</button>
        </a>  --}}
    </div>

    <h1 class="mt-3">Ready for Pickup Orders</h1>
    <table class="table">
        <thead>
            <tr>
                <th>Order No.</th>
        ...
        <tbody>
            @if (count($orders) == 0)
                <tr>
                    <td colspan="6" class="text-center">No orders ready for pickup.</td>
                </tr>
            @endif
            @foreach ($orders as $order)
                <tr>
                    ...
                    <td class="d-flex">
                        <button class="btn btn-info mr-2" data-bs-toggle="modal" data-bs-target="#viewStatusModal{{$order->id}}">View Details</button>
                        <form action="{{url('/pickUpToProcess', $order->id)}}" method="POST">
                            @csrf
                            @method('PUT')
                            <input type="hidden" name="status" value="delivery">
                            <button class="btn btn-primary mx-1" type="submit">Pick up</button>
                        </form>
                    </td>
                </tr>
